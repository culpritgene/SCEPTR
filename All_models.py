...
### we start from the simplistic lstm reader model which attempts to reconstruct word from the latent representation
### obtained out of terminal hidden layer (info on all letters collected)
...
class autoencoder_rnn2(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, latent_space_dim,
                 common_latent_space_dim, mixture_size, embedding_dim_VJ, Vgenes_size, Jgenes_size, alphabet_size, no_borders=False):
        super(autoencoder_rnn2, self).__init__()

        self.no_borders = no_borders
        self.bucket = []  ### just to store some interim values
        self.hidden_dim = hidden_dim
        ...
        self.embedding = nn.Embedding(alphabet_size, embedding_dim)
        self.lstm_in_f = nn.LSTM(embedding_dim, hidden_dim, 3)
        self.lstm_in_r = nn.LSTM(embedding_dim, hidden_dim, 3)
        #### we will use an admixture system starting with small addition from the latent space combining two RNNs
        #### two RNN hidden outputs (dim=128) -> Linear conjunction into common latent space (dim~128?)
        #### two separate decoders, eating corresponding hidden outputs but concatenated with 2+2 neurons representing
        #### embedded Vs/Js and another ~4 spawning from common latent through two independent Linear layers.
        ...
        self.batchnorm_f = nn.BatchNorm1d(hidden_dim)
        self.batchnorm_r = nn.BatchNorm1d(hidden_dim)
    ...
    def encode(self, x, batch_size):
        ...
        if self.no_borders:
            self.bucket.append(embedding[-1, :, :].unsqueeze(0))  ### we reconstruct sequence from the end
            self.bucket.append(embedding_r[-1, :, :].unsqueeze(0))

        lstm_out, lstm_in_hidden_f = self.lstm_in_f(embedding.view(len(x), batch_size, -1),
                                                    self.hidden_in_f)  ### here we translate all sequence at once!
        lstm_out, lstm_in_hidden_r = self.lstm_in_f(embedding_r.view(len(x), batch_size, -1),
                                                    self.hidden_in_r)  ### here we translate all sequence at once!
        lstm_in_hidden = torch.cat((lstm_in_hidden_f[0], lstm_in_hidden_r[0]), dim=2)
        self.common_latent_space_data = F.Relu( F.dropout( self.common_latent_space(lstm_in_hidden), 0.3) )
        self.latent_space_f_data = F.Relu( F.dropout(self.latent_space_f(  self.batchnorm_f( lstm_in_hidden_f[0])), 0.6) )
        self.latent_space_r_data = F.Relu( F.dropout(self.latent_space_r(  self.batchnorm_r(lstm_in_hidden_r[0])), 0.6) )
    ...
    def decode(self, input_length, latent_space_f, latent_space_r, batch_size):
        ...
        if self.no_borders:
            previous_letter_f = self.bucket[0]  # .view(1,batch_size,-1)
            previous_letter_r = self.bucket[1]  # .view(1,batch_size,-1)
        # print('from embedding - first and last symbols:')
        #             print(previous_letter_f.shape, previous_letter_r.shape)
        else:
            previous_letter_f = torch.LongTensor([all_letters.index('X') for x in range(
                batch_size)])  ### lets try to reversely reconstruct from end of sequence symbol
            previous_letter_f = self.embedding(previous_letter_f).view(1, batch_size, -1)

            previous_letter_r = torch.LongTensor([all_letters.index('B') for x in range(
                batch_size)])  ### lets try to reversely reconstruct from end of sequence symbol
        ...
        if self.no_borders:
            detrimental = 1
        for i in range(input_length - detrimental):  ### change this for generator later!
            current_out_f, lstm_out_hidden_f = self.reconstructor_f(previous_letter_f.view(1, batch_size, -1),
                                                                    lstm_out_hidden_f)
            current_out_r, lstm_out_hidden_r = self.reconstructor_r(previous_letter_r.view(1, batch_size, -1),
                                                                    lstm_out_hidden_r)
        ...
        if not self.no_borders:
            # print('Adding last letter')
            last_letter_f = torch.LongTensor([all_letters.index('B') for x in range(
                batch_size)])  ### lets try to reversely reconstruct from end of sequence symbol
            last_letter_f = self.embedding(last_letter_f).view(1, batch_size, -1)
            last_letter_r = torch.LongTensor([all_letters.index('X') for x in range(
                batch_size)])  ### lets try to reversely reconstruct from end of sequence symbol
    ...
    def forward(self, x, Vg, Jg, tick, batch_size):
        ...
        reconstructed_sequence_f, reconstructed_sequence_r = self.decode(len(x), latent_space_f, latent_space_r,
                                                                         batch_size)
        self.bucket = []  #### clear bucket
...
class autoencoder_rnn3(nn.Module):
    ...
    def __init__(self, alphabet, all_letters, Vgenes_size, Jgenes_size, embedding_dim, hidden_dim, hidden_dim2, latent_space_dim,
...
#### letters using cosine simularity, but this might introduce too much complication
    ...
    def decode(self, input_length, latent_space_f, latent_space_r, batch_size): 
        ...
        previous_letter_f = torch.LongTensor([self.all_letters.index('X'*1) for x in range(batch_size)]) ### lets try to reversely reconstruct from end of sequence symbol
        previous_letter_f = self.embedding(previous_letter_f).view(1,batch_size,-1)
        
        previous_letter_r = torch.LongTensor([self.all_letters.index('B'*1) for x in range(batch_size)]) ### lets try to reversely reconstruct from end of sequence symbol
        previous_letter_r = self.embedding(previous_letter_r).view(1,batch_size,-1)
        ...
        last_letter_f = torch.LongTensor([self.all_letters.index('B'*1) for x in range(batch_size)]) ### lets try to reversely reconstruct from end of sequence symbol
        last_letter_f = self.embedding(last_letter_f).view(1,batch_size,-1)
        last_letter_r = torch.LongTensor([self.all_letters.index('X'*1) for x in range(batch_size)]) ### lets try to reversely reconstruct from end of sequence symbol
        last_letter_r = self.embedding(last_letter_r).view(1,batch_size,-1)
...
class autoencoder_rnn4(nn.Module):
    """ This is kmer model with character base level of kmer readings and concatenation of character embedding with kmer"""
    def __init__(self, alphabet, all_letters, Vgenes_size, Jgenes_size, embedding_dim,
                 hidden_dim_char, hidden_dim_kmer,
                 latent_space_dim, common_latent_space_dim, mixture_size, embedding_dim_VJ):
        super(autoencoder_rnn4, self).__init__()

        self.alphabet = list(alphabet)
        self.all_letters = all_letters
        self.hidden_dim_kmer = hidden_dim_kmer
        self.hidden_dim_char = hidden_dim_char

        self.hidden_char_f = self.init_hidden(1, hidden_type='char')
        self.hidden_char_r = self.init_hidden(1, hidden_type='char')
        self.hidden_kmer_f = self.init_hidden(1, hidden_type='kmer')
        self.hidden_kmer_r = self.init_hidden(1, hidden_type='kmer')
        self.latent_space_f_data = None
        self.latent_space_r_data = None
        self.common_latent_space_data = None
        ...
        self.embedding_kmers = nn.Embedding(len(alphabet), embedding_dim)
        self.embedding = nn.Embedding(len(all_letters), hidden_dim_char)

        self.lstm_in_f_kmer = nn.LSTM(embedding_dim, hidden_dim_kmer)
        self.lstm_in_r_kmer = nn.LSTM(embedding_dim, hidden_dim_kmer)

        self.lstm_in_f_char = nn.LSTM(embedding_dim, hidden_dim_char)
        self.lstm_in_r_char = nn.LSTM(embedding_dim, hidden_dim_char)
        ...
        self.Firing = Firing()
        self.Firing.char_in_f_out = None
        self.Firing.char_in_r_out = None
        self.Firing.kmer_in_f_out = None
        self.Firing.kmer_in_r_out = None
        ...
        self.common_latent_space = nn.Linear( (hidden_dim_kmer+hidden_dim_char) * 2, common_latent_space_dim)

        self.latent_space_f = nn.Linear( (hidden_dim_kmer+hidden_dim_char), latent_space_dim)
        self.latent_space_r = nn.Linear( (hidden_dim_kmer+hidden_dim_char), latent_space_dim)

        self.mixture_to_f = nn.Linear(common_latent_space_dim, mixture_size)
        self.mixture_to_r = nn.Linear(common_latent_space_dim, mixture_size)

        self.reconstructor_f = nn.LSTM(hidden_dim_char, hidden_dim_char)
        self.reconstructor_r = nn.LSTM(hidden_dim_char, hidden_dim_char)
    ...
    def encode(self, x, batch_size):
        ...
        self.embedding_data = embedding.view(len(x), batch_size, -1)
        self.embedding_data_r = embedding_r.view(len(x), batch_size, -1)

        for kmr in self.embedding_data:
            for char in kmr:
                self.Firing.char_in_f_out, self.hidden_char_f = self.lstm_in_f_char(char, self.hidden_char_f)
            self.hidden_kmer_f = torch.cat([self.hidden_kmer_f, self.hidden_char_f])
            self.Firing.kmer_in_f_out, self.hidden_kmer_f = self.lstm_in_f_kmer(kmr, self.hidden_kmer_f)

        for kmr in self.embedding_data_r:
            for char in kmr:
                self.Firing.char_in_r_out, self.hidden_char_r = self.lstm_in_r_char(char, self.hidden_char_r)
            self.hidden_kmer_r = torch.cat([self.hidden_kmer_r, self.hidden_char_r])
            self.Firing.kmer_in_r_out, self.hidden_kmer_r = self.lstm_in_r_kmer(kmr, self.hidden_kmer_r)

        lstm_in_hidden = torch.cat((self.hidden_kmer_f[0], self.hidden_kmer_r[0]), dim=2)
        self.common_latent_space_data = F.dropout(F.tanh(self.common_latent_space(lstm_in_hidden)), 0.5)
        self.latent_space_f_data = self.latent_space_f(self.hidden_kmer_f[0])
        self.latent_space_r_data = self.latent_space_r(self.hidden_kmer_r[0])

        return self.latent_space_f_data, self.latent_space_r_data, embedding, embedding_r

    def decode(self, input_length, latent_space_f, latent_space_r, batch_size):
        previous_letter_f = torch.LongTensor([self.all_letters.index('X' * 1) for x in range(
            batch_size)])  ### lets try to reversely reconstruct from end of sequence symbol
        previous_letter_f = self.embedding(previous_letter_f).view(1, batch_size, -1)

        previous_letter_r = torch.LongTensor([self.all_letters.index('B' * 1) for x in range(
            batch_size)])  ### lets try to reversely reconstruct from end of sequence symbol
        previous_letter_r = self.embedding(previous_letter_r).view(1, batch_size, -1)
        ...
        last_letter_f = torch.LongTensor([self.all_letters.index('B' * 1) for x in range(
            batch_size)])  ### lets try to reversely reconstruct from end of sequence symbol
        last_letter_f = self.embedding(last_letter_f).view(1, batch_size, -1)
        last_letter_r = torch.LongTensor([self.all_letters.index('X' * 1) for x in range(
            batch_size)])  ### lets try to reversely reconstruct from end of sequence symbol
        last_letter_r = self.embedding(last_letter_r).view(1, batch_size, -1)
    ...
    def forward(self, x, Vg, Jg, len_, batch_size):
        ...
        latent_space_f, latent_space_r, embedded_f, embedded_r = self.encode(x, batch_size)
        self.common_latent_mu = self.common_latent_mu_layer(self.common_latent_space_data)
        self.common_latent_sigma = self.common_latent_sigma_layer(self.common_latent_space_data)
        self.common_latent_space_data = self.reparametrize(self.common_latent_mu, self.common_latent_sigma)
        reconstructed_sequence_f, reconstructed_sequence_r = self.decode(len(x), latent_space_f, latent_space_r, batch_size)
        return latent_space_f, latent_space_r, reconstructed_sequence_f, reconstructed_sequence_r, embedded_f, embedded_r
    ...
    def init_hidden(self, batch_size, hidden_type='char'):
        # The axes semantics are (num_layers, minibatch_size, hidden_dim)
        if hidden_type=='char':
            return (torch.zeros(1, batch_size, self.hidden_dim_char),
                    torch.zeros(1, batch_size, self.hidden_dim_char))
        if hidden_type=='kmer':
            return (torch.zeros(1, batch_size, self.hidden_dim_kmer),
                    torch.zeros(1, batch_size, self.hidden_dim_kmer))
